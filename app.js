...
const validThemes = ["executive", "personal-brand", "technical-grid", "midnight-glass", "emerald-glass"];
...
const buildCvLines = (data) => {
  ...
  pushCenter(profile.title || "Senior Product Operations Engineer", true, 10);
  push("", false, true);
...
const setupThemeSwitcher = () => {
  ...
  const applyTheme = (theme) => {
    const nextTheme = validThemes.includes(theme) ? theme : "executive";
  ...
  let initialTheme = fromQuery;
  if (!initialTheme) {
    try {
      initialTheme = localStorage.getItem("portfolioTheme");
    } catch (_) {
      initialTheme = "executive";
    }
  }
  applyTheme(initialTheme || "executive");
